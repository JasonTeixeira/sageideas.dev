...
export async function GET(
  ...
  const { data: doc } = await admin
    .from('documents')
    .select(
      'id, title, body_md, organization_id, status, signed_at, signature_name, signature_timestamp, signature_ip, signature_user_agent, signature_hash',
    )
    .eq('id', id)
    .maybeSingle();
  ...
  if (!isAdmin) {
    // Confirm requesting user belongs to the org.
